...
namespace Catalog.Service.EventHandlers
{
        ...
        INotificationHandler<ProductInStockUpdateStockCommand>
    {
        ...
        public async Task Handle(ProductInStockUpdateStockCommand notification, CancellationToken cancellationToken)
        {
            ...
            List<ProductStock> stocks = await _context.Stock.Where(x => products.Contains(x.ProductId)).ToListAsync();

            _logger.LogInformation("--- Retrieve products from database");

            foreach (ProductInStockUpdateItem item in notification.Items)
            {
                ProductStock entry = stocks.SingleOrDefault(x => x.ProductId == item.ProductId);
